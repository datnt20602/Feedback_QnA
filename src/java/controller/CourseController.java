...
public class CourseController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean modQna = false;
        if ((boolean) session.getAttribute("isStudent")) {
            if (request.getParameter("mod") != null) {
                modQna = true;
            }
            Student s = (Student) session.getAttribute("user");
            list = cd.getAllByStudent(s.getStudentId());
        } else {
            if (request.getParameter("mod") != null) {
                modQna = true;
            }
            Lecture l = (Lecture) session.getAttribute("user");
            list = cd.getAllByLecture(l.getLectureId());
        }
        request.setAttribute("data", list);
        request.setAttribute("modQna", modQna);
